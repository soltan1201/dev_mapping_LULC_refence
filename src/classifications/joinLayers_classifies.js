...
var layerCampoRFV1 = imgClass.filter(ee.Filter.eq('layer', 'areasCampestres'))
                             .filter(ee.Filter.eq('classificador', 'RF'))
...
var maskCampo = layerCampoRFV1.eq(12);
var maskWater = layerCampoRFV1.eq(33);
var maskWaterGTB = classGTBvect.eq(33).focalMax(pmtrosfocalMax).rename('water');
// update pixels of Campo from layer Campo(grassland)
var classModGTB = classGTBvect.where(maskCampo.eq(1).and(classGTBvect.eq(4).or(classGTBvect.eq(15))), 12); 
classModGTB = classModGTB.where(maskWaterGTB.eq(1).and(maskWater.eq(1)), 33); // atualizando water from layer campo
...
Map.addLayer(layerCampoRFV1.selfMask(), vis.visclassCC, 'layerCampo RF V1');
// Map.addLayer(maskWaterGTB.selfMask(), {min:0, max: 1}, 'mask Water')
...
if (exportar){
    ...
    funct_export_Img(classModGTB, 'mapa_GTB_mod_S2_refence_2020_v2', regiaoCaat);
