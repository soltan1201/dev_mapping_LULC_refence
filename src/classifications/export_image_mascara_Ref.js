...
var lst_trim_planet = ['trimestre_4'];  //'trimestre_1','trimestre_2','trimestre_3',
...
var classRF = imgClass.filter(ee.Filter.eq('version', version))
                        .filter(ee.Filter.eq('classificador', 'RF')).first();
var classGTBv3 = imgClass.filter(ee.Filter.eq('version', '4'))
                        .filter(ee.Filter.eq('classificador', 'GTB')).first();                        
var classRFv3 = imgClass.filter(ee.Filter.eq('version', '4'))
                        .filter(ee.Filter.eq('classificador', 'RF')).first();
...
lst_trim_planet.forEach(function(nperiodo){
    ...
    Map.addLayer(imgColGeodatin, vis.mosaic_Median_Wet, "periodo_Wet", false);
...
Map.addLayer(classRFv3, vis.visclassCC, 'class RF v4' )
Map.addLayer(classGTBv3, vis.visclassCC, 'class GTB v3')

// Paint all the polygon edges with the same number and width, display.
var outline = ee.Image().toByte().paint({
                  featureCollection: regiaoCaat, 
                  color: 1,  width: 3 });
Map.addLayer(outline, {palette: 'FF0000'}, param.carta_selected + 'subGride 0', false);

var name_export= 'mascara_agricultura';
var idAssetBase = 'projects/mapbiomas-arida/coletaMapaRef';
// processoExportar(agricultura, name_export, idAssetBase)

name_export= 'mascara_area_urbana';
// processoExportar(urbano, name_export, idAssetBase)

var imgAg  = agricultura.reduceToImage(['classe'], ee.Reducer.first())
Map.addLayer(imgAg, {min:0, max: 1}, 'agro');
name_export= 'mascara_area_agricultura_image';
processoExportarImage(imgAg, name_export, idAssetBase, regiaoCaat);

var imgCity  = urbano.reduceToImage(['classe'], ee.Reducer.first())
Map.addLayer(imgCity, {min:0, max: 1}, 'Urbano');
name_export= 'mascara_area_urbana_image';
processoExportarImage(imgCity, name_export, idAssetBase, regiaoCaat);